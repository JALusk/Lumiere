...
def calculate_bc_flux_h01(obs_group):
    ...
    return BCBolometricFlux(
        ...
        np.mean([x.uncertainty for x in fbc]),
        np.mean([x.time for x in fbc]),
    )
...
def compute_bolometric_correction(method_name, obs1, obs2):
    ...
    if range_min <= color <= range_max:
        ...
        raise InvalidColor(
            ...
            + "the {2} method.".format(obs1.band.name, obs2.band.name, method_name)
        )
...
def retrieve_bc_method_data(method_name, path=bc_color_fits):
    with open(path, "r") as bc_color_data_file:
        ...
        try:
            ...
            raise InvalidBCMethod(
                f"Bolometric correction method {method_name} not found"
            )
...
def get_bc_method_coefficients(method_data, band1, band2):
    try:
        ...
        raise InvalidFilterCombination(
            f"{band1.name} - {band2.name} not an allowable color for this method"
        )
...
def apply_bolometric_correction(bc, observed_magnitude):
    ...
    mbol_uncertainty = math.sqrt(
        bc.uncertainty**2 + observed_magnitude.uncertainty**2
    )
