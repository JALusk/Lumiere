...
class TestBolometricCorrection(unittest.TestCase):
    ...
    def setUp(self):
        self.BC = fbc.BolometricCorrection(3.0, 0.1)
    ...
    def test_apply_bolometric_correction(self):
        expected = fbc.BolometricMagnitude(5.0, 0.1, 1234.5)
        result = fbc.apply_bolometric_correction(self.BC, self.obs)
...
class TestRetrieveBolometricCorrectionData(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.method_data = fbc.retrieve_bc_method_data(self.method_name)

    @patch('builtins.open', mock_open(read_data = bc_color_json_data), create=True)
    def test_invalid_method_name(self):
        method = 'TEST'
        with self.assertRaises(fbc.InvalidBCMethod):
            result = fbc.retrieve_bc_method_data(method)

    def test_retrieve_bc_color_coefficients(self):
        expected = [-0.823, 5.027, -13.409, 20.133, -18.096, 9.084, -1.950]
        result = fbc.get_bc_method_coefficients(self.method_data, self.B_band, self.V_band)
        self.assertEqual(expected, result)

    def test_retrieve_zeropoint(self):
        expected = -11.64
        result = fbc.get_bc_method_zeropoint(self.method_data)
        self.assertEqual(expected, result)

    def test_retrieve_bc_color_coefficients_raises_invalid_filter_combination(self):
        with self.assertRaises(fbc.InvalidFilterCombination):
            result = fbc.get_bc_method_coefficients(self.method_data, self.U_band, self.B_band)

    def test_retrieve_bc_color_range(self):
        expected_min = -0.2
        expected_max = 1.65
        result = fbc.get_bc_method_range(self.method_data, self.B_band, self.V_band)
        self.assertEqual(expected_min, result[0])
        self.assertEqual(expected_max, result[1])

    def test_retrieve_bc_color_data_rms(self):
        expected = 0.113
        result = fbc.get_bc_method_rms(self.method_data, self.B_band, self.V_band)
        self.assertEqual(expected, result)
...
class TestComputeBolometricCorrectionPolynomial(unittest.TestCase):
    ...
    def test_compute_bolometric_correction(self):
        expected = -0.0170
        result = fbc.compute_bolometric_correction(self.method, self.B_obs, self.V_obs)
        self.assertAlmostEqual(expected, result.value, 3)

    def test_compute_bolometric_correction_invalid_range(self):
        with self.assertRaises(fbc.InvalidColor):
            ...
            result = fbc.compute_bolometric_correction(self.method, B_obs, V_obs)
...
class TestComputePolynomial(unittest.TestCase):
    ...
    def test_zero_coefficients(self):
        ...
        color = 1.0
        expected = 0
        result = fbc.compute_polynomial(color, coefficients)
        self.assertEqual(expected, result)
...
class TestComputePolynomialDerivative(unittest.TestCase):
    ...
    def test_simple_example(self):
        ...
        color = 2.0
        expected = 62
        result = fbc.compute_polynomial_derivative(color, coefficients)
        self.assertEqual(expected, result)
...
class TestBolometricCorrectionTechniqueH01(unittest.TestCase):
    ...
    def test_calculate_bc_flux_BV(self):
        expected = 7.519E-12
        result = fbc.calculate_bc_flux_h01(self.multi_band_photometryBV)
        print(expected, result.value)
        self.assertAlmostEqual(expected, result.value, delta = 0.01E-12)

    def test_calculate_bc_flux_VI(self):
        expected = 8.060E-12
        result = fbc.calculate_bc_flux_h01(self.multi_band_photometryVI)
        print(expected, result.value)
        self.assertAlmostEqual(expected, result.value, delta = 0.01E-12)
